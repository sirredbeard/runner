...
Image publishing is best effort and never blocks a binary release, so image tags can lag the version list above. Building the arm64 image needs a self-hosted arm64 Windows host with Docker, because GitHub-hosted `windows-11-arm` runners ship no Docker engine and no Windows Containers feature.
