...
- Runs L0 tests (on x64 only, ARM64 testing skipped)
...
4. **Publish-image jobs:** (Currently disabled - Windows Container Docker builds have issues on GitHub Actions)
   - Would build and push Windows Container images to ghcr.io
...
❌ Windows Docker image builds in GitHub Actions (requires proper Windows Container runner configuration)
...
- **v2.331.0** - First automated release with complete CI/CD pipeline
  - Removed dev.sh modifications (use upstream vswhere logic)
  - Added SHA256 validation
  - Split Docker builds to CD workflow
  - Verified package contents match upstream exactly

## Future Improvements

- [ ] Fix Windows Container Docker image builds in GitHub Actions
- [ ] Add package size validation (detect unexpected bloat)
- [ ] Consider multi-stage Docker build to reduce image size
