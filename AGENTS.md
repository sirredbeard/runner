...
4. **Publish-image jobs:** Builds and publishes Windows Container images
   ...
   - The amd64 image builds on `windows-latest`; the arm64 image builds natively on a
     `windows-11-arm` hosted runner (free for public repos) so the produced image is a
     genuine arm64 Windows container, not a mislabeled amd64 one
...
❌ L0 tests on ARM64 are not currently run in CI (could be enabled via `windows-11-arm` runners)

> Note: GitHub now provides native `windows-11-arm` hosted runners (free for public repos),
> which the release workflow uses to build the arm64 Windows container image. The arm64
> binary ZIP is still produced via cross-compilation on `windows-latest`.
